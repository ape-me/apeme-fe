...
function tradeSheet({side,item,stock,holding,resume}){
  ...
  const feesNotable=()=>!!rent()||(q.inUsd>0&&fees()/q.inUsd>0.05);
  ...
  const quoted=()=>`<div style="padding-top:28px">
      ${row(side==='buy'?'Receive':'Selling',side==='buy'?`≈ ${youGet()} · ${fmt.cash(q.swapUsd)}`:fmt.qty(sellQty(),item.symbol),'outcome')}
    ...
    ${rent()?`<div class="notice" style="margin-top:14px;display:block;line-height:19px">First time holding ${esc(item.symbol)}: ${fmt.cash(rent())} of this is a one-time network fee to open the token in your wallet. Next time it's just ${fmt.cash(fees()-rent())}.</div>`:''}
