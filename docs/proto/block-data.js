...
async function requestData(path){
 ...
 try{const r=await fetch(API+path,{signal:controller.signal});if(!r.ok)throw new Error(r.status);const d=await r.json();cache.set(path,{t:Date.now(),d});failedPaths.delete(path);state.online=failedPaths.size===0;updateDataStatus();return d;}
