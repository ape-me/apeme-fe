...
GET /v1/wallet/:address?activity=50                    → {address, totalUsd, solUsd, stocksUsd, memesUsd, costUsd, pnlUsd, realizedUsd, holdings:[Holding], activity:[Activity], asOf}   Portfolio tab (address = the Privy wallet)
GET /v1/stocks?collection=ai                            → same as /stocks, filtered to one collection id; every Stock also carries tags:["ai","mag7"] for client-side chips
